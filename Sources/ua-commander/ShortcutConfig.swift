...
struct KeyBinding: Codable {
    ...
    static func make(keyCode: UInt32, carbonModifiers: UInt32) -> KeyBinding {
        KeyBinding(keyCode: keyCode, modifiers: carbonModifiers,
                   displayString: buildDisplay(keyCode: keyCode, carbonModifiers: carbonModifiers))
    }
}

struct ShortcutConfig: Codable {
    var volumeUp:   KeyBinding
    var volumeDown: KeyBinding
    var mute:       KeyBinding
    var dim:        KeyBinding

    // ⌘⌥↑/↓/M/D — works on every Mac keyboard, no PageUp/Down required
    static let `default` = ShortcutConfig(
        volumeUp:   .make(keyCode: UInt32(kVK_UpArrow),   carbonModifiers: UInt32(cmdKey | optionKey)),
        volumeDown: .make(keyCode: UInt32(kVK_DownArrow), carbonModifiers: UInt32(cmdKey | optionKey)),
        mute:       .make(keyCode: UInt32(kVK_ANSI_M),    carbonModifiers: UInt32(cmdKey | optionKey)),
        dim:        .make(keyCode: UInt32(kVK_ANSI_D),    carbonModifiers: UInt32(cmdKey | optionKey))
    )
    ...
    static func load() -> ShortcutConfig {
        guard let data = UserDefaults.standard.data(forKey: defaultsKey),
              let c = try? JSONDecoder().decode(ShortcutConfig.self, from: data)
        else { return .default }
        return c
    }
...
func buildDisplay(keyCode: UInt32, carbonModifiers: UInt32) -> String {
    carbonModifierSymbols(carbonModifiers) + keyCodeName(keyCode)
}

func carbonModifierSymbols(_ mods: UInt32) -> String {
    var s = ""
